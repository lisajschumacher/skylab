...
class PointSourceLLH(object):
    ...
    def _select_events(self, src_ra, src_dec, **kwargs):
        ...
        min_dec = np.maximum(-np.pi / 2., src_dec - self.delta_ang) 
        max_dec = np.minimum(np.pi / 2., src_dec + self.delta_ang) 
        ...
        if self.mode == "box":
            ...
            cosFact = np.minimum(np.cos(min_dec), np.cos(max_dec))
            dPhi = np.minimum(2. * np.pi, 2. * self.delta_ang / cosFact)
            
            #Find all events within (ra,dec) tolerance
            mask=[]
            for i,src_ra_i in enumerate(src_ra):
                ra_dist = np.fabs((self._ev["ra"] - src_ra_i + np.pi) % (2. * np.pi) - np.pi)
                ra_mask = ra_dist < dPhi[i]/2.
                mask.append(np.logical_and(ra_mask, dec_mask[i][exp_mask]))
    ...
    def llh(self, **fit_pars):
        ...
        # in weights
        if src_w_grad is not None:
            grad_S = np.dot(w_tot_grad.transpose(), self._ev_S) / self._ev['B'] - SoB * np.sum(w_tot_grad)
...
class MultiPointSourceLLH(PointSourceLLH):
    ...
    def llh(self, **fit_pars):
        ...
        New implementation:
        P(m | gamma) = sum_dec P(m, dec | gamma)
        where sum_m sum_dec P(m, dec | gamma) == 1
        P(m, dec | gamma) is relative detection efficiency given for specific sample m and source location dec
