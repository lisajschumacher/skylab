...
def startup(NN=1, multi=False, ncpu=4, **kwargs):
    n = 4
    Nexp = 10000 // NN
    NMC = 500000 // NN
    if multi:
        llh = data.multi_init(n, Nexp, NMC, ncpu=ncpu, **kwargs)
        mc = dict([(i, data.MC(NMC, i+1)) for i in range(n)])
    else:
        llh = data.init(Nexp, NMC, seed=1, ncpu=ncpu, **kwargs)
        mc = data.MC(NMC, seed=2)
