...
def rotate(ra1, dec1, ra2, dec2, ra3, dec3):
    ...
    assert(len(ra1) == len(dec1)
                == len(ra2) == len(dec2)
                == len(ra3) == len(dec3))
    # This is the angle between v1 and v2
    alpha = np.arccos(np.cos(ra2 - ra1) * np.cos(dec1) * np.cos(dec2)
                      + np.sin(dec1) * np.sin(dec2))
    # These are the three directions in cartesian coordinates
    ...
    vec3 = np.vstack([np.cos(ra3) * np.cos(dec3),
                      ...
                      np.sin(dec3)]).T
    # Calculation of the normed rotation axis, orthogonal to v1 and v2
...
def rotate_around(ra1, dec1, ra2, dec2):
    r"""Rotation matrix for rotation of (ra1, dec1) around (ra2, dec2).
    ...
    # This is the angle between v1 and v2
    alpha = np.random.uniform(0., np.pi*2., size=len(ra1))
    # These are the three directions in cartesian coordinates
    vec1 = np.vstack([np.cos(ra1) * np.cos(dec1),
                      np.sin(ra1) * np.cos(dec1),
                      np.sin(dec1)]).T
    nvec = np.vstack([np.cos(ra2) * np.cos(dec2),
                      np.sin(ra2) * np.cos(dec2),
                      np.sin(dec2)]).T
    # Calculation of the normed rotation axis, orthogonal to v1 and v2
    norm = np.sqrt(np.sum(nvec**2, axis=1))
    ...
    R = np.array([(1.-np.cos(a)) * nTn_i + np.cos(a) * one + np.sin(a) * nx_i
                  for a, nTn_i, nx_i in zip(alpha, nTn, nx)])
    vec = np.array([np.dot(R_i, vec_i.T) for R_i, vec_i in zip(R, vec1)])
